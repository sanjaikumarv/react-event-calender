                    ...
                    <div className="absolute top-full left-0 mt-2 bg-white rounded-lg shadow-xl border border-slate-200 z-20 min-w-[120px] max-h-60 overflow-y-auto">
                        {data.map((month, index) => (
                            <button
                                ...
                                className={`w-full text-left px-4 py-2 text-sm hover:bg-slate-50 transition-colors first:rounded-t-lg last:rounded-b-lg ${state === index ? "bg-blue-50 text-blue-600 font-medium" : "text-slate-700"
                                    }`}
                            >
                                {month}
                            </button>
